...
select cast( 'NEW' || ',' || rec.BILL_ID || ',' || '' || ',' ||  'FINACLE_CBS_COUNTERPARTY' || ',' || 'CBS_LOANS_DEPOSITS' || ',' || TO_CHAR(rec.db_stat_date, 'YYYYMMDD') || ',' || 'Finacle' || ',' || 'Finacle' || ',' ||CASE
    ...
    ELSE NULL END
 || ',' || 'StructuredFlows' || ',' || 'Discount' || ',' || 'PREPAID' || ',' || 'FIXED' || ',' || 'REC' || ',' || to_char(rec.VFD_BOD_DATE, 'YYYYMMDD') || ',' || to_char(rec.maturity, 'YYYYMMDD') || ',' || '' || ',' || '' || ',' || '' || ',' || CURRENCY || ',' || abs(rec.Loan_balance) || ',' || 'TRUE' || ',' || 'TRUE' || ',' || rec.INTEREST_RATE || ',' || '' || ',' || '' || ',' || '' || ',' || '' || ',' || '' || ',' || 'ACT/360' || ',' || 'MOD_FOLLOW' ||',' ||'ZC'|| ',' ||'FALSE'|| ',' || '' || ',' || '' || ',' || '' || ',' || '' || ',' || '' || ',' || '' || ',' || 'NEAREST' || ',' || 'NEAREST' || ',' || '' || ',' || '' || ',' || '' || ',' || '' || ',' || '' || ',' || '' || ',' || '' || ',' || '' || ',' || '' || ',' || '' || ',' || '' || ',' || '' || ',' ||'' || ','  || 'Finacle_Liquidity'||','||FINACLECPTYCIFID ||','|| REPLACE(FINACLECPTYNAME, ',', '-')  ||','|| FINACLECPTYTYPE || ',' || 'Bullet' as VARCHAR2(4000 CHAR))   data
from (
SELECT
